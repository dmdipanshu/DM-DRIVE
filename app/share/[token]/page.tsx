...
export default function SharePage({ params }: { params: { token: string } }) {
    ...
    const getPreviewUrl = () => {
        return `/api/share/download?token=${params.token}${password ? `&password=${encodeURIComponent(password)}` : ''}`;
    };
    ...
    const renderPreview = () => {
        ...
        if (file.mimeType.includes("image")) {
            return (
                <div className="w-full h-48 md:h-64 rounded-2xl overflow-hidden mb-6 bg-black/20 flex items-center justify-center relative group">
        ...
        if (file.mimeType.includes("video")) {
            return (
                <div className="w-full h-48 md:h-64 rounded-2xl overflow-hidden mb-6 bg-black/50">
                    <video
                        src={previewUrl}
                        controls
        ...
        if (file.mimeType.includes("audio")) {
            return (
                <div className="w-full h-24 rounded-2xl overflow-hidden mb-6 bg-black/20 flex items-center justify-center p-4">
                    <audio
                        src={previewUrl}
                        controls
                        className="w-full"
        ...
        if (file.mimeType.includes("pdf")) {
            return (
                <div className="w-full h-64 md:h-80 rounded-2xl overflow-hidden mb-6 bg-white">
                    <object
                        data={previewUrl}
    ...
    return (
        <div className="min-h-screen mesh-gradient flex items-center justify-center p-4">
            ...
            <motion.div
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                className="w-full max-w-lg glass-dark rounded-3xl overflow-hidden relative z-10" // Increased from max-w-md to max-w-lg for better preview space
            >
                {/* Header gradient */}
